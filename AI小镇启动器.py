...
# 模拟/工具子进程使用的 Python 解释器：GUI 依赖 PyQt5（base 环境有），
# 而 start.py 等依赖 zhipuai/llama_index（仅 AItown 环境有），二者不在同一环境。
# 找不到 AItown 环境时回退到当前解释器（冻结模式下命令会被映射为 exe，不受影响）。
SIM_PYTHON = r"D:\conda_envs\AItown\python.exe"
if not os.path.isfile(SIM_PYTHON):
    SIM_PYTHON = sys.executable
DEFAULT_TEMPLATE_AGENT = "如鸢密探"
...
class MainWindow(QMainWindow):
    ...
    def start_simulation(self) -> bool:
        ...
        cmd = [SIM_PYTHON, "-u", start_abs, "--name", sim, "--start", "20240213-09:30", "--step", str(self.step_total), "--stride", "10"]
    ...
    def resume_simulation(self):
        ...
        start_abs = os.path.abspath(START_PY_PATH)
        cmd = [SIM_PYTHON, "-u", start_abs, "--name", sim, "--start", "20240213-09:30", "--step", steps, "--stride", "10", "--resume"]
    ...
    def _compress_and_open(self, sim_name: str):
        ...
        self._execute_command_in_thread(
            [SIM_PYTHON, "-u", comp, "--name", sim_name],
    ...
    def trigger_generate_images(self, on_done=None):
        ...
        self._execute_command_in_thread(
            [SIM_PYTHON, "-u", script, "--sim_name", self.last_sim_name],
    ...
    def _run_process(self, task_id: str, label: str, program: str, args: List[str], on_success=None, on_failure=None) -> bool:
        ...
        try:
            ...
            try:
                ...
                self.log(f"Python (GUI): {sys.executable}")
                self.log(f"Python (模拟): {SIM_PYTHON}")
